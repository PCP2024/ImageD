...
from imaged.dataio.imageLoader import ImageLoader, ImageLoadException
...
class TestImageLoader(unittest.TestCase):
    ...
    def tearDown(self):
        self.loader.close_image(image=self.loader.load_image())

    def test_load_image(self):
        image = self.loader.load_image()
        self.assertIsInstance(image, Image.Image)
        self.assertEqual(image.mode, "RGB")
        self.assertEqual(image.size, (271, 270))

    def test_print_image_details(self):
        image = Image.new("RGB", (271, 270))
